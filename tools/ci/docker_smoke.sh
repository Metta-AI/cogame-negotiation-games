...
# Substitute: negotiation-games, coworld-negotiation-games, 3.
#
...
#
# env:
#   SMOKE_IMAGE                image, if not given as $1        (coworld-negotiation-games:ci)
#   SMOKE_SLUG                 game slug                        (negotiation-games)
#   SMOKE_GAME_BIN             game entrypoint                  (/bin/negotiation-games)
#   SMOKE_PLAYER_BIN           player entrypoint                (/bin/negotiation-games-player)
...
#   SMOKE_CONFIG_JSON          JSON object merged over the cert fixture's
#                              game_config (never num_agents/players/tokens)
...
image="${1:-${SMOKE_IMAGE:-coworld-negotiation-games:ci}}"
slug="${SMOKE_SLUG:-negotiation-games}"
...
# SMOKE_CONFIG_JSON (negotiation-games delta on the coworld-builder template):
# a JSON object merged OVER the certification fixture's game_config. The cert
# fixture is deliberately tiny so `coworld certify` fits its 60 s default,
# but the smoke has to produce a replay long enough for the wasm-viewer soak
# and has to play the episode length the design note pins. The seat count is
# NOT taken from here -- num_agents, the roster and the tokens still come
# from certification.game_config, and every seat-count check below is
# unchanged.
_override = (os.environ.get("SMOKE_CONFIG_JSON") or "").strip()
if _override:
    _extra = json.loads(_override)
    if not isinstance(_extra, dict):
        raise SystemExit("SMOKE_CONFIG_JSON must be a JSON object")
    for _key in ("num_agents", "players", "tokens"):
        _extra.pop(_key, None)
    config.update(_extra)
    print(f"SMOKE_CONFIG_JSON applied: {json.dumps(_extra)}")
