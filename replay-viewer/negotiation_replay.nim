## Negotiation Games static replay viewer, wasm side.
##
## JS hands the raw replay bytes to neg_load_replay; this module parses
...
proc negLoadReplay(data: ptr uint8, length: cint): cint
    {.exportc: "neg_load_replay", cdecl.} =
  try:
    lastError = ""
    let replay = parseJson(bytesFromPointer(data, int(length)))
    var config = defaultGameConfig()
    config.matches = replay["config"]{"matches"}.getInt(6)
    config.maxTurns = replay["config"]{"maxTurns"}.getInt(10)
    ...
    payload = $ %*{
      "type": "replay",
      "protocol": replay{"protocol"}.getStr(ReplayProtocol),
      "names": replay["names"],
...
proc negPayloadPointer(): ptr uint8 {.exportc: "neg_payload_ptr", cdecl.} =
  if payload.len == 0:
    nil
  else:
    cast[ptr uint8](payload[0].addr)

proc negPayloadLength(): cint {.exportc: "neg_payload_len", cdecl.} =
  cint(payload.len)

proc negErrorPointer(): ptr uint8 {.exportc: "neg_error_ptr", cdecl.} =
  if lastError.len == 0:
    nil
  else:
    cast[ptr uint8](lastError[0].addr)

proc negErrorLength(): cint {.exportc: "neg_error_len", cdecl.} =
  cint(lastError.len)
