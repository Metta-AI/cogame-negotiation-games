// negotiation-games static replay shell: fetches the replay named by
...
(function () {
  ...
  function start(module, bytes) {
    ...
    var ok = module._neg_load_replay(ptr, bytes.length);
    module._free(ptr);
    if (!ok) {
      fail(readString(module, module._neg_error_ptr(),
        module._neg_error_len()) || "wasm rejected the replay");
      return;
    }
    var payload = JSON.parse(
      readString(module, module._neg_payload_ptr(),
        module._neg_payload_len())
    );
    var loading = document.getElementById("loading");
    if (loading) loading.style.display = "none";
    document.documentElement.removeAttribute("data-replay-error");
    NegotiationRenderer.attachReplay({
      ...
      assetBase: "./assets",
      payload: payload,
      // FIRST DRAWN FRAME, not a bare requestAnimationFrame at this call
      // site: the attribute is set and only THEN is `ready` posted, so a
      // host that samples on the bridge can never catch an unpainted shell
      // (chorus, 2026-08-24).
  ...
  function load() {
    ...
    caption(attempt > 1 ? "RETRYING REPLAY… (attempt " + attempt + ")" :
      "LOADING REPLAY…");
    if (!modulePromise) {
      modulePromise = NegotiationReplayModule().catch(function (error) {
